...
class ProductView(View):
 def get(self, request):
    ...
    footwears=Product.objects.filter(category='FO')
    watches=Product.objects.filter(category='WO')
    shirts=Product.objects.filter(category='SH')
    if request.user.is_authenticated:
      totalitem=len(Cart.objects.filter(user=request.user))
    return render(request, 'shop/home.html',{'topwears':topwears, 'bottomwears':bottomwears, 'mobiles':mobiles, 'laptops':laptops,'accessories':accessories,
    'footwears':footwears,'watches':watches,'shirts':shirts,'totalitem':totalitem})
...
def add_to_cart(request):
 ...
 product=Product.objects.get(product_slug=product_id)
...
def laptop(request, data=None):
  if data == None:
    laptops = Product.objects.filter(category='L')
  elif data == 'HP' or data == 'Dell' or data == 'Apple' or data == 'Acer':
...
def bottomwear(request, data=None):
  if data == None:
    bottomwears = Product.objects.filter(category='BW')
  elif data == 'Ben Martin' or data == 'levis' or data == 'Pepe Jeans' or data == 'Urbano' or data == 'Wrangler' or data == 'Jack & Jones' or data == 'Van Heusen Academy' or data == 'Campus Sutra':
    print(data)
...
def topwear(request, data=None):
  if data == None:
    topwears = Product.objects.filter(category='TW')
  elif data == 'Jack & Jones' or data == 'Allen Solly' or data == 'Shasmi' or data == 'Lymio' or data == 'Siril' or data == 'Hanumntra':
    print(data)
...
# Registration Form
class CustomerRegistrationFor(View):
 def get(self, request):
  frm = CustomerRegistrationForm()
  return render(request, 'shop/customerregistration.html',{'fom':frm})
 def post(self, request):
  form = CustomerRegistrationForm(request.POST)
  if form.is_valid():
    messages.success(request,'Congratulations!! Registered Successfully')
    form.save()
  return render(request, 'shop/customerregistration.html',{'fom':form})
